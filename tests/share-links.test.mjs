...
test('short links preserve all key bits and old fragments remain compatible',()=>{
 for(let i=0;i<100;i++){const key=randomBytes(32).toString('hex');for(const kind of ['e','r']){const link=shortLink(kind,key);assert.equal(link.length,47);assert.equal(linkKey(link.split('#')[1]),key);assert.equal(linkKey('#key='+key),key);assert.equal(new URL(link,'https://hyu-martini.site').pathname,'/'+kind+'/');}}
});
