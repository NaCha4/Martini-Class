...
test('semester folders isolate member forms, edits and exported roster',async({page,baseURL})=>{
 ...
 await expect(page.locator('.semester-tree [aria-current=page]')).toHaveText('2027-1');await expect(page.locator('.roster-title')).toHaveCount(0);await expect(page.locator('main')).not.toContainText(name);
