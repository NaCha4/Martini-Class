...
test('member removal is confirmed, scoped to the semester and reversible',async({page,baseURL})=>{
 ...
 await row().getByRole('button',{name:'명부에서 제거',exact:true}).click();const d=page.getByRole('dialog');await expect(d).toContainText(name);await expect(d).toContainText('2026-2');await d.getByRole('button',{name:'닫기',exact:true}).last().click();await expect(row()).toHaveCount(1);
 ...
 await page.reload();await expect(row()).toHaveCount(0);await page.getByRole('link',{name:'제거한 부원 보기'}).click();await expect(row()).toHaveCount(1);
 await row().getByRole('button',{name:'명부에 복구'}).click();await d.getByRole('button',{name:'명부에 복구',exact:true}).click();await expect(d).toHaveCount(0);await expect(row()).toHaveCount(0);
 await page.getByRole('link',{name:'현재 명부로 돌아가기'}).click();await expect(row()).toHaveCount(1);
