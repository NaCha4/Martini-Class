...
- 부원 라운지: https://hyu-martini.site/members/
- 외부인 출입·문의 처리: https://hyu-martini.site/admin/requests/
- 공개 화면은 GitHub Pages, 인증·데이터는 Firebase 프로젝트 martini-class-d4d69를 사용한다.

## 사용할 수 있는 기능

- 소개·활동·공지·가입 안내와 카카오톡 오픈채팅 연결
- 부원 라운지에서 현재 행사·공지 확인, 행사 신청, 외부인 출입 승인 요청, 문의 및 결과 조회
- 동아리 가입 신청은 운영진이 별도 Google Form으로 처리하며 라운지에서는 새 가입 신청을 받지 않는다. 기존 `/join` 오픈채팅 안내와 설정은 유지한다.
- 이전에 라운지로 접수된 가입 기록은 삭제하지 않으며 개인 확인 링크와 운영진 처리 화면에서 계속 조회·처리할 수 있다.
- 현재 학기 명부의 이름·학번·연락처 확인 후 2시간 동안 부원 기능 이용; 출입 신청은 간부 승인 이후 이용
- 일반 부원 로그인 없이 행사 링크로 신청, 개인 확인 링크로 조회·취소·입금 확인 요청
- 선착순 정원, 대기 접수, 승급 제안·수락·거절, 수기 출석, 참가비·환불 기록
- 부원 명부와 등록 학기, 직접 만드는 역할별 업무 권한·임기
- 주류·재료·도구 재고, 입출고·개봉·10% 잔량·실사·위치 이동·이력
- 수입·지출·회비 장부, 중복 처리와 과다 납부·환불 방지
- 회의·안건·논의·결정·담당 업무를 Firestore에 저장하고 수정 버전 보관
- 사용자 정의 역할 생성·수정과 임원 배정
...
2. **학기 · 운영 설정**에서 현재 학기, 활동 장소, 소개, 가입 오픈채팅 링크와 문의 채널을 입력한다. 회비 금액·기간을 미리 정하지 않는다. 개인정보 처리방침은 기존 GitHub 원문을 사용한다.
3. 가입·회비 확인이 끝난 부원을 명부에 등록한다. 별도 회비 체크나 활동 상태 선택은 없다.
...
- 외부인 출입 신청과 간부 승인은 부원 라운지에서 제공한다. QR, 자동 알림, 팀 신청, 투표, 파일 첨부는 제공하지 않는다.
