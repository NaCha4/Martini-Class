...
async function eventPage(ctx,id){
 ...
 const applicationForm=canApply?'<form data-form="apply" aria-labelledby="apply-title" aria-describedby="apply-help"><p class="help">별도 로그인 없이 신청할 수 있습니다. <span aria-hidden="true">*</span> 표시는 필수 항목입니다.</p>'+field('name','이름','',{required:true,autocomplete:'name',maxLength:40})+field('studentId','학번','',{required:true,maxLength:30,inputMode:'numeric',autocomplete:'off',spellcheck:false})+e.questions.map((q,i)=>field('answer'+i,q,'',{type:'textarea',rows:2,required:true,maxLength:500})).join('')+field('consent','개인정보 수집·이용과 취소 안내를 확인했습니다',false,{type:'checkbox',required:true})+'<a class="privacy-link" href="/privacy" target="_blank" rel="noopener noreferrer">개인정보 안내 보기 (새 탭)</a><p role="alert" class="form-error"></p><button type="submit" class="button full">'+(willWait?'대기 신청하기':'신청하기')+icon('arrow-right')+'</button><p class="help">신청 결과에서 참가 등록·대기 여부를 확인하고, 개인 확인 링크를 꼭 보관해 주세요.</p></form>':'<div class="notice-warning">'+esc(stateNote)+'</div>'+button('신청 상태 새로고침','public-refresh',{class:'button secondary full'});
 return shell('<div class="application-layout"><section class="event-public-detail"><span class="eyebrow">'+esc(label(e.type))+'</span><span class="badge '+esc(statusClass)+'">'+esc(statusText)+'</span><h1>'+esc(e.title)+'</h1><div class="event-facts"><p>'+icon('calendar-days')+'<span>'+scheduleDate(e.startsAt)+'<small>종료 '+scheduleDate(e.endsAt)+'</small></span></p><p>'+icon('map-pin')+'<span>'+esc(e.location)+'</span></p><p>'+icon('wallet')+'<span>'+(e.fee?money(e.fee):'참가비 없음')+'</span></p><p>'+icon('users')+'<span>등록 '+e.registered+' / '+e.capacity+'명'+(e.waitlist?' · 대기 '+e.waiting+'명':'')+'</span></p></div>'+'<section class="application-description"><h2>활동 안내</h2>'+textBlock(e.description||'별도 준비물 안내가 없습니다.')+'</section><hr><h2>신청과 취소</h2><p class="help">신청 시작 '+scheduleDate(e.opensAt)+'<br>신청 마감 '+scheduleDate(e.closesAt)+'<br>자율 취소 마감 '+scheduleDate(e.cancelUntil)+'</p>'+textBlock(e.policy)+'</section><aside class="apply-card" aria-labelledby="apply-title"><span class="eyebrow">행사 신청</span><h2 id="apply-title">'+(canApply?(willWait?'대기 신청':'참가 신청'):statusText)+'</h2>'+(canApply?'<p class="help" id="apply-help">이번 행사 학기의 부원 명부에 등록되어 있으면 신청할 수 있습니다. 이름과 학번을 명부와 같게 입력해 주세요.</p>':'')+(canApply&&willWait?'<p class="event-state-note">'+esc(stateNote)+'</p>':'')+applicationForm+'</aside></div>');
}
