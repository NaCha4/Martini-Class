...
const permissions={events:['owner','chair','education','execution','finance'],members:['owner','chair','execution','finance'],finance:['owner','chair','finance'],inventory:['owner','chair','education','execution'],meetings:['owner','chair','education','execution','finance','publicity'],decisions:['owner','chair','education','execution','finance','publicity'],content:['owner','chair','publicity'],settings:['owner','chair'],admins:['owner'],audit:['owner','chair'],privacy:['owner']};
const navigation=[['','layout-dashboard','오늘의 운영'],['events','calendar-days','행사 · 교육'],['members','users-round','부원 명부'],['inventory','package','재고 관리'],['finance','wallet','회비 · 정산'],['meetings','notebook-pen','회의록'],['decisions','list-checks','결정 · 할 일'],['content','megaphone','공지 · 활동'],['settings','settings-2','학기 · 운영 설정'],['admins','shield-check','임원 권한'],['privacy','shield-check','학기말 정보 정리'],['audit','history','변경 이력']];
const can=(ctx,kind)=>permissions[kind]?.includes(ctx.state.profile?.role);
...
function heading(eyebrow,title,description,action=''){return '<div class="page-heading"><div><h1>'+title+'</h1><p>'+description+'</p></div>'+action+'</div>';}
function toolbar(kind,choices=[]){return '<div class="toolbar"><label class="search-box">'+icon('search')+'<input data-search placeholder="이름이나 내용으로 검색" aria-label="목록 검색"></label>'+(choices.length?'<select data-filter aria-label="상태 필터"><option value="all">전체 상태</option>'+choices.map(c=>'<option value="'+c+'">'+esc(label(c))+'</option>').join('')+'</select>':'')+'<span id="filtered-count" class="muted"></span></div>';}
...
function next(ctx,kind){return ctx.state.pages?.[kind]?.nextCursor?'<div class="pagination">'+button('다음 100개 불러오기','load-more',{id:kind,class:'button secondary'})+'</div>':'';}
...
async function home(ctx){
 ...
 const metrics=[['calendar-days','다가오는 행사',events.length,'events'],['users-round','활동 부원',(data.members||[]).filter(m=>m.status==='active').length,'members'],['package','확인할 재고',items.length,'inventory'],['list-checks','진행 중 할 일',tasks.length,'decisions']].filter(m=>can(ctx,m[3]));
 return heading('YOUR CLUB, AT A GLANCE','운영 현황','이번 학기의 행사, 부원, 재고와 진행 중인 업무입니다.')+
 ...
 '<div class="dashboard-grid"><section class="panel"><div class="panel-title"><h2>다가오는 행사</h2><a data-nav href="/admin/events">전체 보기 '+icon('arrow-right')+'</a></div>'+(events.length?events.slice(0,4).map(e=>'<a data-nav href="/admin/events/'+e.id+'" class="schedule-row"><div class="date-tile"><b>'+new Date(e.startsAt).getDate()+'</b><small>'+new Intl.DateTimeFormat('ko-KR',{month:'short'}).format(new Date(e.startsAt))+'</small></div><div><h3>'+esc(e.title)+'</h3><p>'+esc(date(e.startsAt,true))+' · '+esc(e.location)+'</p></div>'+badge(e.status)+'</a>').join(''):empty('예정된 행사가 없습니다','첫 행사를 만들어 준비를 시작해 보세요.'))+'</section>'+
 '<section class="panel"><div class="panel-title"><h2>함께 처리할 일</h2><a data-nav href="/admin/decisions">전체 보기 '+icon('arrow-right')+'</a></div>'+(tasks.length?tasks.slice(0,5).map(d=>'<button class="task-row" data-action="decision-view" data-id="'+d.id+'"><span class="task-dot"></span><div><h3>'+esc(d.title)+'</h3><p>'+esc(d.owner||'담당 미정')+' · '+(d.dueAt?date(d.dueAt):'기한 미정')+'</p></div>'+badge(d.status)+'</button>').join(''):empty('남은 할 일이 없습니다','회의에서 정한 후속 업무를 여기에 모아두세요.'))+'</section></div>'+
...
async function list(ctx,kind){
 ...
 if(kind==='events'){
  const writable=['owner','chair','education','execution'].includes(ctx.state.profile.role);
  ...
  (rows.length?'<div class="event-grid">'+rows.map(e=>'<a class="event-card" href="/admin/events/'+e.id+'" data-nav data-searchable="'+esc(e.title+' '+e.location)+'" data-status="'+e.status+'"><div class="event-visual '+e.type+'">'+icon(e.type==='class'?'martini':e.type==='meeting'?'users-round':'sparkles')+'<span>'+esc(label(e.type))+'</span>'+badge(e.status)+'</div><div class="event-content"><h2>'+esc(e.title)+'</h2><p>'+icon('calendar-days')+date(e.startsAt,true)+'</p><p>'+icon('map-pin')+esc(e.location)+'</p><div class="event-bottom"><span>등록 <b>'+e.registered+'</b> / '+e.capacity+'명</span><span>'+money(e.fee)+'</span></div><div class="capacity-bar"><span style="width:'+Math.min(100,e.registered/e.capacity*100)+'%"></span></div></div></a>').join('')+'</div>':empty('첫 행사를 준비해 보세요','공유 링크를 받은 부원이 로그인 없이 신청할 수 있습니다.'))+next(ctx,kind);
 }
 if(kind==='members'){
  const heads=['부원','소속','학번 · 연락처','학기','활동','회비','관리'];
  return heading('THE PEOPLE OF MARTINI','부원 명부','입력한 명부를 기준으로 행사 신청 자격을 확인합니다.',button('부원 등록','member-edit',{icon:'user-plus'}))+toolbar(kind,['active','inactive','withdrawn','graduated'])+
  '<div class="list-meta"><p>활동 중 + 이번 학기 회비 납부 완료 부원만 신청할 수 있습니다.</p>'+button('CSV 내보내기','export',{id:kind,class:'button small secondary',icon:'download'})+'</div>'+
  (rows.length?table(heads,rows.map(m=>row(m,m.name+' '+m.studentId+' '+m.department,[ '<strong>'+esc(m.name)+'</strong><small>'+esc(m.grade?m.grade+'학년':'')+'</small>',esc(m.college)+'<small>'+esc(m.department)+'</small>',esc(m.studentId)+'<small>'+esc(m.phone)+'</small>',esc(m.semester),badge(m.status),badge(m.duesPaid?'paid':'unpaid'),button('수정','member-edit',{id:m.id,class:'button small secondary'})],heads))):empty('명부가 아직 비어 있습니다','부원을 등록하면 회비와 행사 참여를 함께 관리할 수 있습니다.'))+next(ctx,kind);
 }
 if(kind==='inventory'){
 ...
 if(kind==='content'){
  ...
  (rows.length?table(heads,rows.map(c=>row(c,c.title+' '+c.body,[esc(c.title),c.type==='notice'?'공지':'활동 기록',badge(c.published?'final':'draft'),date(c.updatedAt),button('수정','content-edit',{id:c.id,class:'button small secondary'})],heads))):empty('공개할 이야기를 작성해 주세요','초안으로 저장한 글은 홈페이지에 보이지 않습니다.'))+next(ctx,kind);
 }
 if(kind==='admins'){
  const heads=['임원','부서 · 권한','임기 종료','상태','관리'];
  return heading('THE TEAM','임원 권한','Firebase Authentication에 존재하는 계정 UID를 임원으로 등록합니다.',button('임원 등록','admin-edit',{icon:'user-plus'}))+toolbar(kind)+table(heads,rows.map(a=>row(a,a.displayName+' '+a.role,[esc(a.displayName),badge(a.role),date(a.expiresAt),a.active?'사용 가능':'중지',button('수정','admin-edit',{id:a.id,class:'button small secondary'})],heads)));
 }
 if(kind==='audit'){
...
async function eventDetail(ctx,id){
 ...
 const write=['owner','chair','education','execution'].includes(ctx.state.profile.role);
 let html='<a class="back-link" href="/admin/events" data-nav>'+icon('arrow-left')+' 행사 목록</a>'+heading(esc(label(e.type)),esc(e.title),date(e.startsAt,true)+' · '+esc(e.location),write?'<div class="row-actions">'+button('신청 링크','event-link',{id:e.id,class:'button secondary',icon:'link'})+button('편집','event-edit',{id:e.id,icon:'pencil'})+'</div>':'')+
 ...
 '<nav class="tabs">'+[['participants','신청 · 출석'],['detail','안내 · 정책'],['preparation','준비 · 결과']].map(([key,t])=>'<a href="/admin/events/'+id+'?tab='+key+'" data-nav class="'+(tab===key?'active':'')+'">'+t+'</a>').join('')+'</nav>';
 ...
 (rows.length?table(heads,rows.map(a=>row(a,a.name,['<strong>'+esc(a.name)+'</strong><small>'+date(a.createdAt,true)+'</small>',badge(a.status)+(a.status==='waiting'?'<small>접수 순서 '+a.sequence+'</small>':''),badge(a.payment)+'<small>'+money(a.paidAmount)+' 확인</small>',badge(a.attendance),button('처리','application-manage',{id:a.id,class:'button small secondary'})],heads))):empty('아직 신청자가 없습니다','행사 신청 링크를 복사해서 부원들에게 전달해 주세요.'));
}
function login(ctx){
 return '<main class="login-page"><a href="/" data-nav class="brand"><img class="wordmark" src="/assets/wordmark.png" alt="Martini" width="170" height="42"></a><section class="login-card"><span class="eyebrow">임원 로그인</span><h1>마티니 운영실</h1><p>임원 계정으로 로그인해 주세요.</p><form data-form="login">'+field('email','이메일','',{type:'email',required:true,autocomplete:'username'})+field('password','비밀번호','',{type:'password',required:true,autocomplete:'current-password'})+'<p role="alert" class="form-error">'+esc(ctx.state.authError||'')+'</p><button type="submit" class="button full">로그인 '+icon('arrow-right')+'</button></form>'+button('비밀번호 재설정','password-reset',{class:'button ghost full'})+(ctx.state.user?button('다른 계정으로 로그인','logout',{class:'button secondary full'}):'')+(local?'<div class="local-note">로컬 검증 환경 · 실제 데이터와 분리되어 있습니다.'+button('가상 임원으로 확인하기','local-login',{class:'button secondary full'})+'</div>':'')+'</section></main>';
}
export async function renderAdmin(ctx){
 if(!ctx.state.authReady)return '<div class="loading">'+icon('loader-circle')+'로그인 상태 확인 중</div>';
 if(!ctx.state.profile)return login(ctx);
 const part=location.pathname.split('/').filter(Boolean),kind=part[1]||'';
 if(kind && !can(ctx,kind))return '<main class="connection-page"><h1>접근 권한이 없습니다</h1><a href="/admin" data-nav class="button">운영 홈</a></main>';
 const config=(await read(ctx,'settings')).rows[0];ctx.state.settings=config||{semester:'2026-2'};
 let body=kind==='events'&&part[2]?await eventDetail(ctx,part[2]):kind==='settings'?heading('SET UP THE SEMESTER','학기 · 운영 설정','실제 운영 정보와 개인정보 안내를 확인해 주세요.',button('설정 수정','settings-edit',{icon:'pencil'}))+'<section class="panel padded">'+(config?'<div class="detail-grid"><p>현재 학기<br><strong>'+esc(config.semester)+'</strong></p><p>학기 회비<br><strong>'+money(config.duesAmount)+'</strong></p><p>보존 검토 기준일<br><strong>'+date(config.semesterEndsAt)+'</strong></p><p>문의처<br><strong>'+esc(config.contact||'미입력')+'</strong></p></div><hr><h3>소개</h3>'+textBlock(config.intro)+'<hr><h3>개인정보 안내</h3>'+textBlock(config.privacy)+'<hr><h3>학기 종료 정리</h3><p>부원 정보·신청·증빙은 한 학기 기준으로 보존 여부를 검토합니다. 실제 개인정보 삭제는 백업·정산 영향과 대상을 확인한 후 별도 승인 절차로 수행합니다. 현재 화면에서는 운영 데이터를 일괄 삭제하지 않습니다.</p>':empty('첫 학기 설정을 입력해 주세요','문의처와 개인정보 안내를 입력해야 행사 모집을 열 수 있습니다.'))+'</section>':kind?await list(ctx,kind):await home(ctx);
 return '<div class="workspace"><aside class="sidebar"><a href="/admin" data-nav class="side-brand"><img src="/assets/logo.png" alt="" width="42" height="42"><span>MARTINI<small>운영실</small></span></a><div class="side-semester">'+icon('calendar-days')+esc(ctx.state.settings.semester)+'<span>운영 중</span></div><div class="side-label">운영 메뉴</div><nav>'+navigation.filter(([k])=>!k||can(ctx,k)).map(([key,i,t])=>'<a href="/admin'+(key?'/'+key:'')+'" data-nav class="'+(kind===key?'active':'')+'">'+icon(i)+'<span>'+t+'</span>'+(kind===key?'<span class="nav-dot"></span>':'')+'</a>').join('')+'</nav><div class="side-footer"><a href="/" data-nav>'+icon('arrow-up-right')+' 홈페이지 보기</a><button data-action="logout">'+icon('log-out')+' 로그아웃</button></div></aside><div class="workspace-main"><header class="workspace-header"><div class="breadcrumb">마티니 <span>/</span> '+esc(navigation.find(([k])=>k===kind)?.[2]||'운영실')+'</div><div class="account-chip"><span class="avatar">'+esc(ctx.state.profile.displayName.slice(0,1))+'</span><div>'+esc(ctx.state.profile.displayName)+'<small>'+label(ctx.state.profile.role)+'</small></div></div></header>'+(local?'<div class="local-strip">개발 환경 · 가상 데이터</div>':'')+'<main class="workspace-content">'+body+'</main><nav class="mobile-admin-nav">'+navigation.filter(([k])=>['','events','inventory','meetings'].includes(k)&&(!k||can(ctx,k))).map(([key,i,t])=>'<a data-nav href="/admin'+(key?'/'+key:'')+'" class="'+(kind===key?'active':'')+'">'+icon(i)+'<span>'+t+'</span></a>').join('')+button('전체 메뉴','mobile-menu',{class:'mobile-more',icon:'menu'})+'</nav></div></div>';
}
export async function adminAction(ctx,action,id,target){
 if(action==='password-reset'){modal('비밀번호 재설정',field('email','가입한 이메일','',{type:'email',required:true,wide:true}),async f=>{await sendPasswordResetEmail(auth,String(f.get('email')).trim());ctx.toast('등록된 계정이면 비밀번호 재설정 메일이 발송됩니다.');},{submit:'재설정 메일 요청'});return;}
 ...
 if(action==='mobile-menu'){modal('운영 메뉴','<div class="mobile-menu-list wide">'+navigation.filter(([k])=>!k||can(ctx,k)).map(([k,i,t])=>'<a href="/admin'+(k?'/'+k:'')+'" data-nav>'+icon(i)+t+'</a>').join('')+'</div>',null);return;}
