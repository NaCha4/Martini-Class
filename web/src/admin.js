...
async function eventDetail(ctx,id){
 ...
 (rows.length?table(heads,rows.map(a=>row(a,a.name,['<strong>'+esc(a.name)+'</strong><small>'+date(a.createdAt,true)+'</small>',badge(a.status)+(a.status==='waiting'?'<small>접수 순서 '+a.sequence+'</small>':''),...(finance?[badge(a.payment)+'<small>'+money(a.paidAmount)+' 확인</small>']:[]),(write&&e.status!=='cancelled'&&a.status==='registered'?'<button type="button" class="attendance-toggle '+(a.attendance==='present'?'present':'absent')+'" data-attendance-id="'+esc(a.id)+'" data-saved-value="'+(a.attendance==='present'?'present':'absent')+'" aria-label="'+esc(a.name)+' 출석" aria-pressed="'+(a.attendance==='present')+'">'+(a.attendance==='present'?'출석':'불참')+'</button>':badge(a.attendance==='present'?'present':'absent')),'<div class="row-actions">'+button('신청 상세 · 처리','application-manage',{id:a.id,class:'button small secondary'})+(write?deleteButton('applications',a.id,'참가 신청'):'')+'</div>'],heads))):empty('아직 신청자가 없습니다','행사 신청 링크를 복사해서 부원들에게 전달해 주세요.'));
}
function login(ctx){
