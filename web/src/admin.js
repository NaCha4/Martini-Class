...
const scopeEvent=ctx=>['owner','chair','education','execution'].includes(ctx.state.profile?.role);
...
async function home(ctx){
 ...
 const setup=can(ctx,'settings')&&(!ctx.state.settings.id||!ctx.state.settings.contact||!ctx.state.settings.privacy)?'<section class="operations-note">'+icon('settings-2')+'<div><h2>모집 전에 운영 정보를 확인해 주세요</h2><p>현재 학기, 문의처와 개인정보 안내를 입력하면 행사 모집을 준비할 수 있습니다.</p></div><a href="/admin/settings" data-nav class="button secondary">운영 설정 확인</a></section>':'';
