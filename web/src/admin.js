...
async function list(ctx,kind){
 ...
 const {rows}=['finance','members','meetings','decisions'].includes(kind)?await readAll(ctx,kind):await read(ctx,kind);
 if(kind==='events'){
