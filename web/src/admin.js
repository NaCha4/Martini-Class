...
async function list(ctx,kind){
 ...
 if(kind==='decisions'){
  const selected=decisionCategoryId(),scope=decisionCategoryOptions(await readDecisionCategories(ctx),rows,selected);
  rows=scope.rows;
  const writable=!selected||!!scope.category;
  const options=[['','공통 업무 · '+(scope.counts.get('')||0)+'건'],...scope.categories.map(c=>[c.id,c.name+' · '+(scope.counts.get(c.id)||0)+'건'])];
  if(selected&&!scope.category)options.push([selected,'삭제된 카테고리']);
  const context='<section class="decision-context" aria-label="카테고리별 결정 · 할 일"><div class="decision-category-picker"><label for="decision-category">카테고리 선택</label><select id="decision-category" data-decision-category>'+options.map(([id,title])=>'<option value="'+esc(id)+'"'+(id===selected?' selected':'')+'>'+esc(title)+'</option>').join('')+'</select></div>'+button('카테고리 추가','decision-category-add',{class:'button secondary',icon:'plus'})+(scope.category?button('카테고리 삭제','decision-category-delete',{id:scope.category.id,class:'button secondary',icon:'trash-2'}):'')+'<div class="decision-context-copy"><h2>'+esc(scope.category?.name||(selected?'카테고리를 찾을 수 없습니다':'공통 업무'))+'</h2><p>'+(!selected?'카테고리가 없는 결정과 할 일을 관리합니다.':!scope.category?'이 카테고리의 기록은 공통 업무에서 확인할 수 있습니다.':'이 카테고리의 결정과 할 일만 표시합니다.')+'</p>'+(selected&&!scope.category?'<a data-nav class="button small secondary" href="/admin/decisions">공통 업무 보기</a>':'')+'</div></section>';
...
async function eventDetail(ctx,id){
 ...
 (rows.length?table(heads,rows.map(a=>row(a,a.name,['<strong>'+esc(a.name)+'</strong><small>'+date(a.createdAt,true)+'</small>',badge(a.status)+(a.status==='waiting'?'<small>접수 순서 '+a.sequence+'</small>':''),...(finance?[badge(a.payment)+'<small>'+money(a.paidAmount)+' / '+money(billingFee(a))+'</small>'+button(a.isStaff?'관리인원 · '+money(billingFee(a)):'참가비 설정','application-pricing',{id:a.id,class:'button small ghost'})]:[]),(write&&e.status!=='cancelled'&&a.status==='registered'?'<button type="button" class="attendance-toggle '+(a.attendance==='present'?'present':'absent')+'" data-attendance-id="'+esc(a.id)+'" data-saved-value="'+(a.attendance==='present'?'present':'absent')+'" aria-label="'+esc(a.name)+' 출석" aria-pressed="'+(a.attendance==='present')+'">'+(a.attendance==='present'?'출석':'불참')+'</button>':badge(a.attendance==='present'?'present':'absent')),'<div class="row-actions">'+button('신청 상세 · 처리','application-manage',{id:a.id,class:'button small secondary'})+(write?deleteButton('applications',a.id,'참가 신청'):'')+'</div>'],heads))):empty('아직 신청자가 없습니다','행사 신청 링크를 복사해서 부원들에게 전달해 주세요.'));
}
function login(ctx){
