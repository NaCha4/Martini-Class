...
export const removedRoster=()=>location.pathname.replace(/\/$/,'')==='/admin/members'&&new URLSearchParams(location.search).get('removed')==='1';
function memberParams(ctx,kind,params){if(kind!=='members')return params;const semester=params.semester||rosterSemester(ctx),removed=params.removed??removedRoster();if(ctx.state.memberSemester!==semester||ctx.state.memberRemoved!==removed){ctx.state.data.members={};if(ctx.state.pages)delete ctx.state.pages.members;ctx.state.memberSemester=semester;ctx.state.memberRemoved=removed;}return {...params,semester,removed};}
...
async function list(ctx,kind){
 ...
 if(kind==='members'){
  const removed=removedRoster(),term=rosterSemester(ctx),terms=[...new Set([...(await ctx.api('rosterTerms')).rows,term])].sort().reverse();
  ...
  return heading('THE PEOPLE OF MARTINI','부원 명부','학기를 선택하면 해당 학기의 부원 정보를 확인할 수 있습니다.',removed?'':button('부원 등록','member-edit',{icon:'user-plus'}))+tree+'<h2 class="roster-title">'+esc(term)+(removed?' 제거한 부원':' 부원 명부')+'</h2><a data-nav class="text-button" href="/admin/members?semester='+term+(removed?'':'&removed=1')+'">'+(removed?'현재 명부로 돌아가기':'제거한 부원 보기')+'</a>'+toolbar(kind)+
  '<div class="list-meta"><p>'+(removed?'명부에서 제거된 부원입니다. 복구하면 이 학기의 행사 신청 자격이 다시 부여됩니다.':'선택한 학기에 등록됩니다. 다른 학기의 명부는 변경되지 않습니다.')+'</p>'+(removed?'':button('CSV 내보내기','export',{id:kind,class:'button small secondary',icon:'download'}))+'</div>'+
  (rows.length?table(heads,rows.map(m=>row(m,m.name+' '+m.studentId+' '+m.phone+' '+m.department,[ '<button class="title-button" data-action="member-view" data-id="'+esc(m.id)+'"><strong>'+esc(m.name)+'</strong></button><small>'+esc(m.grade?m.grade+'학년':'')+'</small>',esc(m.college)+'<small>'+esc(m.department)+'</small>',esc(m.studentId)+'<small>'+esc(m.phone)+'</small>','<div class="row-actions">'+(removed?button('명부에 복구','member-restore',{id:m.id,class:'button small secondary',disabled:!!m.anonymizedAt}):button('수정','member-edit',{id:m.id,class:'button small secondary'})+button('명부에서 제거','member-remove',{id:m.id,class:'button small secondary'}))+'</div>'],heads))):empty(removed?'제거한 부원이 없습니다':'명부가 아직 비어 있습니다',removed?'이 학기에서 제거한 부원은 이곳에 표시됩니다.':'학번과 연락처가 행사 신청 정보와 일치해야 참가 자격을 확인할 수 있습니다.',removed?'':button('첫 부원 등록','member-edit',{class:'button secondary'})))+next(ctx,kind);
 }
 if(kind==='inventory'){
...
export async function adminAction(ctx,action,id,target){
 ...
 if(action==='load-more'){const current=ctx.state.pages[id];const result=await ctx.api('read',{kind:id,cursor:current.nextCursor,...(id==='members'?{semester:rosterSemester(ctx),removed:removedRoster()}:{})});result.rows.forEach(r=>ctx.state.data[id][r.id]=r);ctx.state.pages[id]={rows:[...current.rows,...result.rows],nextCursor:result.nextCursor};ctx.toast('추가 기록 '+result.rows.length+'개를 불러왔습니다.');return showMoreRows(ctx,id);}
