...
import { decisionEventId, decisionBoardUrl, readDecisionEvents, decisionEventOptions } from './decision-events.js';
...
async function list(ctx,kind){
 ...
 if(kind==='decisions'){
  const selected=decisionEventId(),scope=decisionEventOptions(await readDecisionEvents(ctx),rows,selected);
  rows=scope.rows;
  const writable=!selected||scope.event&&!scope.event.archived;
  const options=[['','공통 업무 · '+(scope.counts.get('')||0)+'건'],...scope.events.map(e=>[e.id,e.title+' · '+e.semester+(e.archived?' · 보관됨':'')+' · '+(scope.counts.get(e.id)||0)+'건'])];
  if(selected&&!scope.event)options.push([selected,'찾을 수 없는 행사']);
  const context='<section class="decision-context" aria-label="행사별 결정 · 할 일"><div class="decision-event-picker"><label for="decision-event">행사 선택</label><select id="decision-event" data-decision-event>'+options.map(([id,title])=>'<option value="'+esc(id)+'"'+(id===selected?' selected':'')+'>'+esc(title)+'</option>').join('')+'</select></div>'+(scopeEvent(ctx)?button('행사 추가','decision-event-add',{class:'button secondary',icon:'plus'}):'')+'<div class="decision-context-copy"><h2>'+esc(scope.event?.title||(selected?'행사를 찾을 수 없습니다':'공통 업무'))+'</h2><p>'+(!selected?'행사에 연결하지 않은 결정과 할 일을 관리합니다.':!scope.event?'다른 행사를 선택해 주세요.':scope.event.archived?'보관된 행사의 기존 기록입니다. 진행 상태를 변경하거나 다른 행사로 옮길 수 있습니다.':'이 행사에 연결된 결정과 할 일만 표시합니다.')+'</p></div></section>';
