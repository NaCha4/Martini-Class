...
function details(r){
 ...
 return '<div class="wide request-detail"><div class="request-detail-head"><span class="eyebrow">'+esc(kinds[r.kind])+'</span>'+status(r)+'</div><h3>'+esc(title(r))+'</h3><dl class="request-facts"><div><dt>신청자</dt><dd>'+esc(applicant.name)+'</dd></div><div><dt>학번</dt><dd>'+esc(applicant.studentId)+'</dd></div><div><dt>접수</dt><dd>'+stamp(r.createdAt)+'</dd></div>'+(r.kind==='visit'?'<div><dt>방문 시작</dt><dd>'+stamp(r.startsAt)+'</dd></div>'+(r.endsAt?'<div><dt>방문 종료</dt><dd>'+stamp(r.endsAt)+'</dd></div>':'')+'<div><dt>외부인</dt><dd>'+esc(r.guestCount)+'명 · '+esc(r.guestNames)+'</dd></div>':r.kind==='join'?'<div><dt>학과 · 학년</dt><dd>'+esc(r.department)+' · '+esc(r.grade)+'</dd></div>':'')+'</dl>'+textBlock(r.kind==='visit'?r.purpose:r.message||'별도 메시지가 없습니다.')+(r.response?'<div class="request-response"><h4>운영진 답변</h4>'+textBlock(r.response)+'<small>'+stamp(r.reviewedAt||r.updatedAt)+'</small></div>':'')+'</div>';
}
