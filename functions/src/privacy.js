 ...
 async function anonymize(data,who){
  ...
  return db.runTransaction(async tx=>{
   ...
   for(const {doc,kind} of plan.records){
    ...
    if(kind==='member')Object.assign(patch,{name:'정보 정리 완료',studentId:'',phone:'',college:'',department:'',grade:'',gender:'',identityHash:FieldValue.delete(),status:'withdrawn',duesPaid:false,revision:(doc.data().revision||0)+1});
