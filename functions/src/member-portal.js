...
export function createMemberPortal({db,col,clock,now,roster,throttle,audit}){
 ...
 async function access(data,ctx){
  const input=parse(z.object({...applicant,sessionKey:key}).strict(),data);
  ...
  return db.runTransaction(async tx=>{
   ...
   const matched=members.filter(member=>live(member)&&member.name===input.name&&normalizePhone(member.phone)===input.phone);
   if(matched.length!==1)fail('permission-denied','이름, 학번, 연락처 또는 현재 활동 자격을 확인할 수 없습니다. 운영진에게 문의해 주세요.');
 ...
 async function portal(data,ctx){
  ...
  return db.runTransaction(async tx=>{
   ...
   return {member:{name:member.name,semester:member.semester},expiresAt,
    ...
    requests:requests.docs.filter(doc=>!doc.data().deletedAt&&!doc.data().anonymizedAt&&fingerprint(doc.data())===fingerprint(member)).map(doc=>safeRequest({...doc.data(),id:doc.id}))};
  },{readOnly:true});
 }
 ...
 async function submit(data,ctx){
  ...
  return db.runTransaction(async tx=>{
   ...
   const {sessionKey,receiptKey,requestId,consent,...fields}=input;
   const actor=member?{name:member.name,studentId:member.studentId,phone:normalizePhone(member.phone)}:{name:input.name,studentId:input.studentId,phone:input.phone};
   ...
   if(prior){
    if(!prior.deletedAt&&!prior.anonymizedAt&&matches(receiptKey,prior.receiptHash)&&prior.payloadHash===payloadHash)return {id:requestId,status:prior.status,request:safeRequest({...prior,id:requestId}),duplicate:true};
 ...
 async function command(data,who){
  ...
  return db.runTransaction(async tx=>{
   ...
   if(record.kind==='visit'&&input.action==='approve'){
    ...
    if(record.semester!==semester||!live(member)||fingerprint(member)!==fingerprint(record))fail('failed-precondition','신청자의 현재 활동 자격 또는 접수 당시 정보를 확인할 수 없습니다.');
   }
