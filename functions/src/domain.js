...
export const schemas={
 member:z.object({...meta,name:required(40),studentId:required(30),phone:required(30),college:text(80).default(''),department:text(80).default(''),grade:text(20).default(''),gender:text(20).default(''),semester:required(30),status:z.enum(['active','inactive','withdrawn','graduated']),duesPaid:z.boolean().default(false)}).strict(),
...
export function ensureScope(admin,scope,now=Date.now()){
 if(!admin?.active||!roles.includes(admin.role)||!admin.expiresAt||!Number.isFinite(Date.parse(admin.expiresAt))||Date.parse(admin.expiresAt)<=now)fail('permission-denied','등록된 임원 계정이 아니거나 임기가 종료되었습니다.');
 if(!scopes[scope]?.includes(admin.role))fail('permission-denied','이 업무를 처리할 권한이 없습니다.');
}
